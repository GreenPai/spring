...
@Repository                      // User1 : Entity , String : @ID Data Type
public interface User1Repository extends JpaRepository<User1, String> {

    // 쿼리 메서드 실습
    // find   /User1/By/Uid
    // select /엔티티/By/uid -> select * from user1 where uid = uid;
    ...
    // JPQL
    @Query("select u1 from User1 as u1 where u1.age <= 25")
    ...
    @Query("select p, c from Parent as p join Child as c on p.pid = c.parent where p.pid= :pid")
